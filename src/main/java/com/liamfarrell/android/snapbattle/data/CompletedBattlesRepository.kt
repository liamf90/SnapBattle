...
(private val lambdaFunctionsInterface: LambdaFunctionsInterface) {
    ...
    suspend fun getCompletedBattles() : AsyncTaskResult<List<Battle>>{
        val battlesRequest = CompletedBattlesRequest()
        return executeAWSFunction {lambdaFunctionsInterface.getCompletedBattles(battlesRequest).sqlResult}
    }
