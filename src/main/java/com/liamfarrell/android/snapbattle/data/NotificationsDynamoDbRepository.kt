...
class NotificationsDynamoDbRepository @Inject constructor(val ddbClient : AmazonDynamoDBClient){
    ...
    suspend fun getNotificationListFromDynamo(startIndex: Int, endIndex: Int): List<Notification> {
        ...
        val notificationIndexList = (startIndex..endIndex).toList().map { getNotificationCountDynamo() - 1 - it }

        if (notificationIndexList.size == notificationsDynamo?.size) {
            ...
            throw Error("Notifications list does not match requested indexes")
        }
